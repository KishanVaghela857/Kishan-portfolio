        ...
        >
            <div className="flex flex-col md:flex-row items-center justify-between w-full max-w-6xl mx-auto gap-12 md:gap-0">
                ...
                <div className="flex-1 flex flex-col items-start justify-center text-left">
                    <motion.p variants={variants("down", 1.100)}
                        initial="hidden"
                        whileInView="show"
                        viewport={{
                            once: true,
                            amount: 0.5,
                        }} className="text-lg text-white mb-2">Hello, It's Me</motion.p>
                    <motion.h1 variants={variants("down", 0.90)}
                        initial="hidden"
                        whileInView="show"
                        viewport={{
                            once: true,
                            amount: 0.5,
                        }} className="text-5xl sm:text-5xl font-extrabold text-white mb-2">KISH△N V△GHEL△</motion.h1>
                    <motion.h2 variants={variants("down", 0.80)}
                        initial="hidden"
                        whileInView="show"
                        viewport={{
                            once: true,
                            amount: 0.5,
                        }} className="text-2xl sm:text-3xl font-bold text-white mb-4">And I'm a <span className="text-cyan-400">Frontend Dev</span></motion.h2>
                    <motion.p variants={variants("down", 0.70)}
                        initial="hidden"
                        whileInView="show"
                        viewport={{
                            once: true,
                            amount: 0.5,
                        }} className="text-gray-300 mb-6 max-w-md">I don't just code websites — I craft experiences. Grind in silence, let the UI shine</motion.p>
                    {/* Social Icons */}
                    <motion.div variants={variants("down", 0.60)}
                        initial="hidden"
                        whileInView="show"
                        viewport={{
                            once: true,
                            amount: 0.5,
                        }} className="flex gap-4 mb-8" style={{ marginTop: '20px' }}>
                        ...
                        <a href="https://www.linkedin.com/in/vaghelakishan/" target='_blank' className="w-10 h-10 flex items-center justify-center rounded-full border-2 border-cyan-400 text-cyan-400 hover:bg-cyan-400 hover:text-white transition-colors duration-200"><svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M19 0h-14c-2.76 0-5 2.24-5 5v14c0 2.76 2.24 5 5 5h14c2.76 0 5-2.24 5-5v-14c0-2.76-2.24-5-5-5zm-7 19h-3v-7h3v7zm-1.5-8.25c-.97 0-1.75-.78-1.75-1.75s.78-1.75 1.75-1.75 1.75.78 1.75 1.75-.78 1.75-1.75 1.75zm13.5 8.25h-3v-4c0-1.1-.9-2-2-2s-2 .9-2 2v4h-3v-7h3v1c.41-.58 1.19-1.41 2.5-1.41 1.93 0 3.5 1.57 3.5 3.5v3.91z" /></svg></a>
                    </motion.div>
                    {/* Download CV Button */}
                    <motion.a variants={variants("up", 0.55)}
                        initial="hidden"
                        whileInView="show"
                        viewport={{
                            once: true,
                            amount: 0.5,
                        }} href="" download={Resume} alt="kishan_vaghela's_resume" className="rounded-full bg-cyan-400 text-[#181924] hover:bg-cyan-500 transition-colors duration-200" style={{ marginTop: '20px', width: '170px', height: '40px', textAlign: 'center', fontFamily: 'sans-serif', justifyContent: 'center', alignContent: 'center', fontWeight: 'bolder' }}>Download CV</motion.a>
                </div>
                {/* Right Side */}
                <motion.div variants={variants("left", 0.55)}
                    initial="hidden"
                    whileInView="show"
                    viewport={{
                        once: true,
